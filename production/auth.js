...
(function gateProtectedPages() {
    try {
        ...
        console.log('Protected page gated: content hidden until approval check completes');
    } catch (e) {
...
const SecurityManager = {
    ...
    async createUserProfile(user, role = 'pending') {
        try {
            ...
            if (existingProfile.exists) {
                ...
                console.log('User profile created with pending role:', user.uid);
                return true;
            }
        } catch (error) {
    ...
    async isUserApproved(uid) {
        try {
            ...
            const isApproved = userData.role !== 'pending' && (userData.isActive !== false);
            console.log('User approval status:', isApproved, 'Role:', userData.role, 'Active:', userData.isActive);
            return isApproved;
        } catch (error) {
    ...
    showPendingApprovalMessage() {
        ...
        console.log('Pending approval message displayed - user blocked from accessing content');
    },
...
(async function immediateBlockingCheck() {
    try {
        ...
        if (!FirebaseAuth.isAuthenticated()) {
            console.log('User not authenticated yet, allowing page to load normally');
            return;
        }
...
window.addEventListener('load', async function() {
    try {
        ...
        // Wait for Firebase Auth to be available
        if (typeof FirebaseAuth === 'undefined') {
            console.log('FirebaseAuth not available on page load, allowing page to load normally');
            return;
        }
        ...
        if (!FirebaseAuth.isAuthenticated()) {
            console.log('User not authenticated on page load, allowing page to load normally');
            return;
        }
        
        console.log('User is authenticated on page load, checking if pending...');
...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        // Wait for Firebase Auth to be available
        if (typeof FirebaseAuth === 'undefined') {
            console.log('FirebaseAuth not available yet, allowing page to load normally');
            return;
        }
...
setInterval(() => {
    try {
        ...
        const blockingMessage = document.getElementById('pending-approval-message');
        if (blockingMessage) {
            return; // Already blocked
        }
        
        // Only check if SecurityManager is available and user is authenticated
        if (typeof SecurityManager !== 'undefined' && FirebaseAuth.getCurrentUser()) {
